...
| **Tasks** | `@mlflow.trace` on every node in `agent/nodes.py` (one span per Section 6 state, not just the D.6 headline list) plus an `mlflow.start_run()` per graph invocation; `notebooks/02_demo_pre_post_circular.py`: runs the full agent twice for `APP-001` via the new `as_of_date_override` input (2026-01-15 "before", 2026-09-18 "after" — no second application row needed); `notebooks/03_run_golden_set_eval.py`: re-runs the 10 engine golden cases (pure Python, imported straight from `engine/tests/test_golden_cases.py`) plus computes concrete agent-layer metrics (retrieval hit, citation correctness, unsupported-claim rate, workflow completion) for the one case with live agent data (`APP-001`). **Schema change surfaced by this phase:** `get_committee_memos` gained an optional third parameter, `p_as_of_date DATE DEFAULT NULL` — without it, the Advisory Pass would see the September post-circular memo even in the "before" run, since memos weren't previously date-filtered at all. Backward compatible (`NULL` = no filter, matching Phase 2/4's existing calls) |
| **Dummy data scope** | None new |
| **Deliverable** | MLflow experiment (`/Shared/credit_platform_assessments_<catalog>`) with 4 traced runs per execution (before/after demo + 2 eval runs); an evaluation report with real computed values, not placeholder categories |
| **Exit criterion** | Verified live in both dev and prod: the demo reproduces the outcome flip as **`eligible` → `manual_review`** — the original design doc's Section 9 prose says "Conditional" for this scenario, but Section 7's JSON contract (the one Phase 3's engine implements and its golden cases test against) says `manual_review` for a failed DSCR rule; Phase 3 treated Section 7 as authoritative, and this demo reproduces that same, already-established behavior rather than reopening it. `policy_version` flips `v1` → `v2`, and the Advisory Pass's committee-memo retrieval is genuinely point-in-time (`MEMO-003`, the September memo, present only in the "after" run); all 10 engine golden cases pass; the agent-layer metrics (retrieval hit, citation correctness, zero unsupported claims, full workflow completion) all pass for the centerpiece case |
...
| `get_committee_memos` | `(applicable_sector STRING, borrower_id STRING DEFAULT NULL, as_of_date DATE DEFAULT NULL) -> ARRAY<STRUCT>` | `gold.committee_memos`, ordered by `memo_date DESC`, optionally filtered to `memo_date <= as_of_date` (added in Phase 5 — see that phase's row for why); **called only by the Advisory Pass**, never by the Explanation Pass or the rules engine |
...
One MLflow run per `application_id` assessment, with nested spans: `identify_application` → `retrieve_data` → `validate_data` → [`flag_missing_data`] → `rules_engine` → [`retrieve_evidence` → `generate_explanation`] ‖ [`run_advisory_pass` → `generate_advisory_notes`] → `synthesize_assessment` → `validate_response` → `human_review` (the two bracketed spans run as sibling/parallel spans, not sequential — implemented as `@mlflow.trace(name=...)` on every node function in `agent/nodes.py`, one span per Section 6 state, not just the headline subset). Every run is tagged with `policy_version` and `eligibility_status` so the pre/post-circular demo is queryable directly from the MLflow UI (filter by `application_id = APP-001`, compare the two runs' tags and their differing `advisory_notes` span output).

**Real platform constraints hit building this:**
- The Databricks Runtime default `mlflow==2.11.4` can't run on this workspace's serverless compute at all — even a plain `mlflow.start_run()` fails with `CONFIG_NOT_AVAILABLE: spark.mlflow.modelRegistryUri`, a Spark Connect config read serverless blocks, and `mlflow.trace` doesn't exist in that version regardless. `pip install -U mlflow` (landing on 3.16.1 as of this build) fixes both — required in every notebook that imports `mlflow`.
- `dbutils.notebook.entry_point.getDbutils().notebook().getContext().tags()` isn't whitelisted on serverless compute (`Py4JSecurityException`) — it can't be used to look up the current username. The demo/eval notebooks use a fixed, catalog-scoped experiment path (`/Shared/credit_platform_assessments_<catalog>`) instead of a per-user one, which is arguably the more correct choice anyway for a project-level experiment that any principal might run.
